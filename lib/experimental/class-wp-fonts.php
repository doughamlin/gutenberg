 ...
 * Class WP_Fonts
 *
 * Extends from the deprecated WP_Webfonts to provide
 * a temporary backwards-compatibility (BC) layer.
 *
 * BACKPORT NOTE: Change extends to `WP_Dependencies`
...
class WP_Fonts extends WP_Webfonts {
	...
	public function __construct() {
		/**
		 * Filters the fonts variation's property defaults.
		 *
		 * @since X.X.X
		 *
		 ...
		 * }
		 */
		$this->variation_property_defaults = apply_filters( 'wp_fonts_variation_defaults', $this->variation_property_defaults );
	...
	private function validate_variation( $variation ) {
		...
		if ( empty( $variation['font-family'] ) || ! is_string( $variation['font-family'] ) ) {
			trigger_error( 'The font-family must be a non-empty string.' );
			return false;
		}
		...
		if ( 'local' === $variation['provider'] ) {
			...
			if ( empty( $variation['src'] ) || ( ! is_string( $variation['src'] ) && ! is_array( $variation['src'] ) ) ) {
				trigger_error( 'The font src must be a non-empty string or an array of strings.' );
				return false;
			}
		...
		if ( ! is_string( $variation['font-weight'] ) && ! is_int( $variation['font-weight'] ) ) {
			trigger_error( 'The font-weight must be a properly formatted string or integer.' );
			return false;
		}
	 ...
	 *
	 * @param string $provider_id The provider to process.
	 * @return array[] Fonts organized by providers.
	 */
	...
	/**
	 * Gets the instance of the provider from the WP_Fonts::$provider_instance store.
	 *
	 * @since X.X.X
	 *
	 ...
	 * @return array Fonts in theme.json structural format.
	 */
